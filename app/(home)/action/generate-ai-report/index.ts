...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientções de como melhorar minha vida financeira. As transações estão dividas por ponto e vigula. A estrutura de cadas uma é {DATA}-{TIPO}-{VALOR}-{GARANTIA}. São elas:
    ${transactions
      .map(
        (transaction) =>
